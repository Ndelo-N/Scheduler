...
class APIClient {
  ...
  async getShifts(scheduleId) {
    return this.get(`/schedules/${scheduleId}/shifts`);
  }

  async createShift(scheduleId, shiftData) {
    return this.post(`/schedules/${scheduleId}/shifts`, shiftData);
  }

  async updateShift(shiftId, shiftData) {
    return this.put(`/shifts/${shiftId}`, shiftData);
  }
  ...
  async getSwapRequests(status = null) {
    const endpoint = status ? `/swaps/requests?status=${status}` : '/swaps/requests';
    return this.get(endpoint);
  }

  async createSwapRequest(swapData) {
    return this.post('/swaps/requests', swapData);
  }

  async updateSwapRequest(swapId, updates) {
    return this.put(`/swaps/requests/${swapId}`, updates);
  }

  async approveSwapRequest(swapId) {
    return this.put(`/swaps/requests/${swapId}/approve`);
  }

  async rejectSwapRequest(swapId, reason) {
    return this.put(`/swaps/requests/${swapId}/reject`, { reason });
  }

  async getSwapOffers(swapRequestId) {
    return this.get(`/swaps/requests/${swapRequestId}/offers`);
  }

  async createSwapOffer(swapRequestId, offerData) {
    return this.post(`/swaps/requests/${swapRequestId}/offers`, offerData);
  }

  // Analytics API methods
  async getScheduleAnalytics(scheduleId) {
    return this.get(`/analytics/schedules/${scheduleId}`);
  }

  async getStudentAnalytics(studentId) {
    return this.get(`/analytics/students/${studentId}`);
  }

  async getSwapAnalytics() {
    return this.get('/analytics/swaps');
  }
  ...
  async importCSV(file) {
    return this.uploadFile(file, '/import/csv');
  }

  async exportSchedule(scheduleId, format = 'csv') {
    return this.get(`/export/schedules/${scheduleId}?format=${format}`);
  }
  ...
  async healthCheck() {
    try {
      const response = await this.get('/health', { skipAuthHandler: true }); // baseURL already '/api' → /health
