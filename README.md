...
function runSchedulingAlgorithm(){
  ...
  shifts.forEach(shift => {
    ...
    // Sort candidates by priority: consistency first, then fairness
...
function isStudentAvailable(studentId, dateStr, startTime, endTime, availability) {
  ...
  return availability[dayOfWeek].some(block => {
...
function rebalance(){
  ...
  let changed = true;
  let iterations = 0;
  const targetGap = 0.5; // aim to equalize within half an hour
  while (changed && iterations < 200){
    iterations++;
    changed = false;

    // Snapshot hours and sort students
    const stats = state.students.map(st=>({id:st.id, name:st.name, h:getH(st.id), cap:capOf(st.id)}));
      ...
      for (const lo of lowList){
        ...
        const trySwap = (filterFn)=>{
          for (const {key, s} of shiftList){
            ...
            if (loRec && loRec.weekly_max_hours && loWeek + addH > loRec.weekly_max_hours) continue;
            if (lo.h + addH > capOf(lo.id)) continue;

            // Prefer swaps that keep weekly consistency for lo
            const dayKey = new Date(`${s.date}T00:00:00`).getDay();
            const consistencyGain = getConsistencyScore(lo.id, dayKey, s.start);

            // Check consecutive hours limit (5 hours max)
            const consecutiveHours = getConsecutiveHours(lo.id, s.date, s.start, s.end);
            if (consecutiveHours > 5) continue;
            ...
            if (ok){
              ...
              const note = consistencyGain>0 ? ' [consistency]' : '';
...
function fillOpenClose(){
  ...
  for (const k of keys){
    ...
    while ((s.assignees?.length||0) < Math.min(cap, s.required||1)){
      const candidates = state.students.filter(st=> !s.assignees.includes(st.id) && validateAssignment(st.id, s).length===0);
